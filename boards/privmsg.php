...
$inbox_url = ( $folder != 'inbox' || $mode != '' ) ? '<a class="MainMenuLink" href="' . append_sid("privmsg.$phpEx?folder=inbox") . '">' . $lang['Inbox'] . '</a>' : $lang['Inbox'];
...
$outbox_url = ( $folder != 'outbox' || $mode != '' ) ? '<a class="MainMenuLink" href="' . append_sid("privmsg.$phpEx?folder=outbox") . '">' . $lang['Outbox'] . '</a>' : $lang['Outbox'];
...
$sentbox_url = ( $folder != 'sentbox' || $mode != '' ) ? '<a class="MainMenuLink" href="' . append_sid("privmsg.$phpEx?folder=sentbox") . '">' . $lang['Sentbox'] . '</a>' : $lang['Sentbox'];
...
$savebox_url = ( $folder != 'savebox' || $mode != '' ) ? '<a class="MainMenuLink" href="' . append_sid("privmsg.$phpEx?folder=savebox") . '">' . $lang['Savebox'] . '</a>' : $lang['Savebox'];
...
$sql_tot = "SELECT COUNT(privmsgs_id) AS total 
	FROM " . PRIVMSGS_TABLE . " ";
$sql = "SELECT pm.privmsgs_type, pm.privmsgs_id, pm.privmsgs_date, pm.privmsgs_subject, u.user_id, u.username, u.user_rank, u.user_posts
	FROM " . PRIVMSGS_TABLE . " pm, " . USERS_TABLE . " u ";
switch( $folder )
{
	case 'inbox':
...
if ( $row = $db->sql_fetchrow($result) )
{
	...
	do
	{
		...
		$msg_username = get_ranked_username($row['user_id'],$row['username'],$row['user_rank'],$row['user_posts']);		
