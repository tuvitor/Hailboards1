...
function prepare_bbcode_template($bbcode_tpl)
{
	...
	$bbcode_tpl['hl_open'] = str_replace('{HL}', '\\1', $bbcode_tpl['hl_open']);
	
	$bbcode_tpl['border_open'] = str_replace('{S_BORDER}', '\\1', $bbcode_tpl['border_open']);

	$bbcode_tpl['bbfont_open'] = str_replace('{BBFONT}', '\\1', $bbcode_tpl['bbfont_open']);
	...
	$bbcode_tpl['image'] = str_replace('{URL}', '\\1', $bbcode_tpl['image']);
...
function bbencode_second_pass($text, $uid)
{
	...
	// highlight
	$text = preg_replace("/\[hl=(\#[0-9A-F]{6}|[a-z]+):$uid\]/si", $bbcode_tpl['hl_open'], $text);
	$text = str_replace("[/hl:$uid]", $bbcode_tpl['hl_close'], $text);

	// border
	$text = preg_replace("/\[border=(\#[0-9A-F]{6}|[a-z]+):$uid\]/si", $bbcode_tpl['border_open'], $text);
	$text = str_replace("[/border:$uid]", $bbcode_tpl['border_close'], $text);
	
	// font
	$text = preg_replace("/\[font=(\#[0-9A-F]{6}|[a-z]+):$uid\]/si", $bbcode_tpl['bbfont_open'], $text);
	$text = str_replace("[/font:$uid]", $bbcode_tpl['bbfont_close'], $text);
	...
	// [strike] and [/strike] for striking out text.
	...
	$patterns[] = "#\[image:$uid\]([^?](?:[^\[]+|\[(?!url))*?)\[/image:$uid\]#i";
	$replacements[] = $bbcode_tpl['image'];
...
function bbencode_first_pass($text, $uid)
{
	...
	// [hl] and [/hl] for setting background highlights
	$text = preg_replace("#\[hl=(\#[0-9A-F]{6}|[a-z\-]+)\](.*?)\[/hl\]#si", "[hl=\\1:$uid]\\2[/hl:$uid]", $text);

	// [border] and [/border] for setting bordered text formatting
	$text = preg_replace("#\[border=(\#[0-9A-F]{6}|[a-z\-]+)\](.*?)\[/border\]#si", "[border=\\1:$uid]\\2[/border:$uid]", $text);
	
	// [font] and [/font] for formatting font face
	$text = preg_replace("#\[font=(\#[0-9A-F]{6}|[a-z\-]+)\](.*?)\[/font\]#si", "[font=\\1:$uid]\\2[/font:$uid]", $text);
	// $text = preg_replace("$\[font=(\#[0-9A-F]{6}|[a-z\-]+)\](.*?)\[/font\]#si", "[font=\\1:$uid]\\2[/font:$uid]", $text);
	...
	$text = preg_replace("#\[image\]((http|ftp|https|ftps)://)([^ \?&=\#\"\n\r\t<]*?(\.(jpg|jpeg|gif|png)))\[/image\]#sie", "'[image:$uid]\\1' . str_replace(' ', '%20', '\\3') . '[/image:$uid]'", $text);
