...
function get_buddies(&$data)
{
	...
	$sql = "SELECT b.buddy_id, u.username AS buddy_name, u.user_email AS buddy_email, u.user_viewemail, u.user_rank, u.user_posts, u.user_allow_viewonline, u.user_session_time
	...
	else
	{
		for( $i = 0; $i < count($buddies_online); $i++ )
		{
			...
			$buddy_name = get_ranked_username($buddies_online['user_id'],$buddies_online[$i]['buddy_name'],$buddies_online[$i]['user_rank'],$buddies_online[$i]['user_posts']);
	...
	else
	{
		for( $i = 0; $i < count($buddies_offline); $i++ )
		{
			...
			$buddy_name = get_ranked_username($buddies_offline['user_id'],$buddies_offline[$i]['buddy_name'],$buddies_offline[$i]['user_rank'],$buddies_offline[$i]['user_posts']);
...
function get_buddy_img(&$user_id, &$buddy_id)
{
	...
	if( $buddy_id != $user_id && $user_id != ANONYMOUS && $buddy_id != ANONYMOUS )
	{
		...
		if( $row = $db->sql_fetchrow($result) )
		{
			...
			$buddy = '<a class="MainMenuLink" href="' . $temp_url . '">' . $lang['Remove_buddy'] . '</a>';
		}
		else
		{
			...
			$buddy = '<a class="MainMenuLink" href="' . $temp_url . '">' . $lang['Add_buddy'] . '</a>';
		}
...
function get_buddies_list(&$data, $mode = '')
{
	...
	$sql = "SELECT b.buddy_id, u.username AS buddy_name, u.user_rank, u.user_posts, u.user_allow_viewonline, u.user_session_time
			FROM " . BUDDIES_TABLE . " b, " . USERS_TABLE . " u
			WHERE b.user_id = " . $data['user_id'] . "
				AND u.user_id = b.buddy_id
			ORDER BY u.username ASC";
