	...
	$mode == 'register' )
{
	...
	$strip_var_list = array('username' => 'username', 'email' => 'email', 'icq' => 'icq', 'aim' => 'aim', 'msn' => 'msn', 'yim' => 'yim', 'website' => 'website', 'location' => 'location', 'occupation' => 'occupation', 'interests' => 'interests', 'real_name' => 'real_name', 'gender' => 'gender', 'company' => 'company', 'picture' => 'picture');
...
// dingo added 'bio' to below array
	$trim_var_list = array('cur_password' => 'cur_password', 'new_password' => 'new_password', 'password_confirm' => 'password_confirm', 'signature' => 'signature', 'bio' => 'bio');
...
// dingo
	if ( $bio != '' )
	{
		if ( $bio_bbcode_uid == '' )
		{
			$bio_bbcode_uid = ( !empty($bio) ) ? make_bbcode_uid() : '';
		}
		$bio = prepare_message($bio, '1', '1', '1', $bio_bbcode_uid);
	}
// end dingo
...
// dingo added user_bio = '" . str_replace("\'", "''", $bio) . "' and user_bio_bbcode_uid = '$bio_bbcode_uid' to sql below
			$sql = "UPDATE " . USERS_TABLE . "
				SET " . $username_sql . $passwd_sql . "user_email = '" . str_replace("\'", "''", $email) ."', user_icq = '" . str_replace("\'", "''", $icq) . "', user_website = '" . str_replace("\'", "''", $website) . "', user_occ = '" . str_replace("\'", "''", $occupation) . "', user_from = '" . str_replace("\'", "''", $location) . "', user_interests = '" . str_replace("\'", "''", $interests) . "', user_real_name = '" . str_replace("\'", "''", $real_name) . "', user_gender = '" . str_replace("\'", "''", $gender) . "', user_company = '" . str_replace("\'", "''", $company) . "', user_picture = '" . str_replace("\'", "''", $picture) . "', user_sig = '" . str_replace("\'", "''", $signature) . "', user_sig_bbcode_uid = '$signature_bbcode_uid', user_bio = '" . str_replace("\'", "''", $bio) . "', user_bio_bbcode_uid = '$bio_bbcode_uid', user_viewemail = $viewemail, user_aim = '" . str_replace("\'", "''", str_replace(' ', '+', $aim)) . "', user_yim = '" . str_replace("\'", "''", $yim) . "', user_msnm = '" . str_replace("\'", "''", $msn) . "', user_attachsig = $attachsig, user_allowsmile = $allowsmilies, user_allowhtml = $allowhtml, user_allowbbcode = $allowbbcode, user_allow_viewonline = $allowviewonline, user_notify = $notifyreply, user_notify_pm = $notifypm, user_popup_pm = $popup_pm, user_timezone = $user_timezone, user_dateformat = '" . str_replace("\'", "''", $user_dateformat) . "', user_lang = '" . str_replace("\'", "''", $user_lang) . "', user_style = $user_style, user_active = $user_active, user_actkey = '" . str_replace("\'", "''", $user_actkey) . "'" . $avatar_sql . "
				WHERE user_id = $user_id";
			if ( !($result = $db->sql_query($sql)) )
			{
		...
		else
		{
			...
			// Get current date
			//
// dingo added in 'user_bio' and 'user_bio_bbcode_uid' to INSERT line and '" . str_replace("\'", "''", $bio) . "' and '$bio_bbcode_uid' to VALUES line in sql below
			$sql = "INSERT INTO " . USERS_TABLE . "	(user_id, username, user_regdate, user_password, user_email, user_regip, user_icq, user_website, user_occ, user_from, user_interests, user_real_name, user_gender, user_company, user_picture, user_sig, user_sig_bbcode_uid, user_bio, user_bio_bbcode_uid, user_avatar, user_avatar_type, user_viewemail, user_aim, user_yim, user_msnm, user_attachsig, user_allowsmile, user_allowhtml, user_allowbbcode, user_allow_viewonline, user_notify, user_notify_pm, user_popup_pm, user_timezone, user_dateformat, user_lang, user_style, user_level, user_allow_pm, user_active, user_actkey)
				VALUES ($user_id, '" . str_replace("\'", "''", $username) . "', " . time() . ", '" . str_replace("\'", "''", $new_password) . "', '" . str_replace("\'", "''", $email) . "', '" . $userdata['session_ip'] . "', '" . str_replace("\'", "''", $icq) . "', '" . str_replace("\'", "''", $website) . "', '" . str_replace("\'", "''", $occupation) . "', '" . str_replace("\'", "''", $location) . "', '" . str_replace("\'", "''", $interests) . "', '" . str_replace("\'", "''", $real_name) . "', '" . str_replace("\'", "''", $gender) . "', '" . str_replace("\'", "''", $company) . "', '" . str_replace("\'", "''", $picture) . "', '" . str_replace("\'", "''", $signature) . "', '$signature_bbcode_uid', '" . str_replace("\'", "''", $bio) . "', '$bio_bbcode_uid', $avatar_sql, $viewemail, '" . str_replace("\'", "''", str_replace(' ', '+', $aim)) . "', '" . str_replace("\'", "''", $yim) . "', '" . str_replace("\'", "''", $msn) . "', $attachsig, $allowsmilies, $allowhtml, $allowbbcode, $allowviewonline, $notifyreply, $notifypm, $popup_pm, $user_timezone, '" . str_replace("\'", "''", $user_dateformat) . "', '" . str_replace("\'", "''", $user_lang) . "', $user_style, 0, 1, ";
...
// dingo
	$bio = stripslashes($bio);
	$bio = ($bio_bbcode_uid != '') ? preg_replace("/:(([a-z0-9]+:)?)$bio_bbcode_uid(=|\])/si", '\\3', $bio) : $bio;
// end dingo
...
// dingo
	$bio_bbcode_uid = $userdata['user_bio_bbcode_uid'];
	$bio = ($bio_bbcode_uid != '') ? preg_replace("/:(([a-z0-9]+:)?)$bio_bbcode_uid(=|\])/si", '\\3', $userdata['user_bio']) : $userdata['user_bio'];
// end dingo
...
if( isset($HTTP_POST_VARS['avatargallery']) || isset($HTTP_GET_VARS['avatargallery']) && !$error )
{
	...
	display_avatar_gallery($mode, $avatar_category, $user_id, $email, $current_email, $coppa, $username, $email, &$new_password, &$cur_password, $password_confirm, $icq, $aim, $msn, $yim, $website, $location, $occupation, $interests, $real_name, $gender, $company, $picture, $signature, $viewemail, $notifypm, $popup_pm, $notifyreply, $attachsig, $allowhtml, $allowbbcode, $allowsmilies, $allowviewonline, $user_style, $user_lang, $user_timezone, $user_dateformat, $userdata['session_id'], $bio);
}
else
{
	...
	if ( $user_avatar_type )
	{
		switch( $user_avatar_type )
		{
			...
			case USER_AVATAR_GALLERY:
				...
				$avatar_img = ( $board_config['allow_avatar_local'] ) ? '<img src="' . $image_start . '' . $avatar_data['avatar_path'] . '" alt="" />' : '';
				break;
		}
	}
