...
else if ( $search_keywords != '' || $search_author != '' || $search_id )
{
	...
	if ( $search_results != '' )
	{
		...
		{
			$sql = "SELECT pt.post_text, pt.bbcode_uid, pt.post_subject, p.*, f.forum_id, f.forum_name, t.*, u.username, u.user_id, u.user_posts, u.user_rank, u.user_sig, u.user_sig_bbcode_uid  
		...
		{
			$sql = "SELECT t.*, f.forum_id, u.user_posts, u.user_rank, u2.username as username2, u2.user_posts as user_posts2, u2.user_rank as user_rank2, f.forum_name, u.username, u.user_id, u2.username as user2, u2.user_id as id2, p.post_username, p2.post_username AS post_username2, p2.post_time 
		...
		for($i = 0; $i < count($searchset); $i++)
		{
			...
			if ( $show_results == 'posts' )
			{
				...
				$poster .= ( $searchset[$i]['user_id'] != ANONYMOUS ) ? get_ranked_username($searchset[$i]['user_id'],$searchset[$i]['username'],$searchset[$i]['user_rank'],$searchset[$i]['user_posts']) : ( ( $searchset[$i]['post_username'] != "" ) ? $searchset[$i]['post_username'] : $lang['Guest'] );
			...
			else
			{
				...
				$topic_author .= ( $searchset[$i]['user_id'] != ANONYMOUS ) ? get_ranked_username($searchset[$i]['user_id'],$searchset[$i]['username'],$searchset[$i]['user_rank'],$searchset[$i]['user_posts']) : ( ( $searchset[$i]['post_username'] != '' ) ? $searchset[$i]['post_username'] : $lang['Guest'] );
				...
				$last_post_author = ( $searchset[$i]['id2'] == ANONYMOUS ) ? ( ($searchset[$i]['post_username2'] != '' ) ? $searchset[$i]['post_username2'] . ' ' : $lang['Guest'] . ' ' ) : get_ranked_username($searchset[$i]['id2'],$searchset[$i]['username2'],$searchset[$i]['user_rank2'],$searchset[$i]['user_posts2']);
		...
		$template->assign_vars(array(
			...
			'HB_PAGINATION' => hb_generate_pagination($base_url, $total_match_count, $per_page, $start),
