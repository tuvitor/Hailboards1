...
if ( $is_auth['auth_mod'] )
{
	$s_auth_can .= sprintf($lang['Rules_moderate'], "<a class='MainMenuRow' href=\"modcp.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['session_id'] . '">', '</a>');
...
//UNCOMMENT WHEN hb_generate_pagination is added into functions.php >>
$hb_pagination = ( $highlight != '' ) ? hb_generate_pagination("viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;postdays=$post_days&amp;postorder=$post_order&amp;highlight=$highlight", $total_replies, $board_config['posts_per_page'], $start) : hb_generate_pagination("viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;postdays=$post_days&amp;postorder=$post_order", $total_replies, $board_config['posts_per_page'], $start);
...
for($i = 0; $i < $total_posts; $i++)
{
	$poster_id = $postrow[$i]['user_id'];
 	$poster = ( $poster_id == ANONYMOUS ) ? ( ($postrow[$i]['poster'] != '' ) ? $postrow[$i]['poster'] . ' ' : $lang['Guest'] . ' ' ) : get_ranked_username($postrow[$i]['poster_id']);
        $hb_poster = ( $poster_id == ANONYMOUS ) ? $lang['Guest'] : $postrow[$i]['username'];
	...
	//
	                        case USER_AVATAR_GALLERY:
                                $sql = "SELECT *
                                        FROM " . AVATAR_TABLE . "
                                        WHERE avatar_id = " . $postrow[$i]['user_avatar'];
                                ...
                                $poster_avatar = ( $board_config['allow_avatar_local'] ) ? '<img src="' . $image_start . '' . $avatar_data['avatar_path'] . '" alt="" border="0" />' : '';
	...
	else if ( $postrow[$i]['user_rank'] )
	{
		for($j = 0; $j < count($ranksrow); $j++)
		{
			if ( $postrow[$i]['user_rank'] == $ranksrow[$j]['rank_id'] && $ranksrow[$j]['rank_special'] )
			{
				...
				$hb_rank_image = ( $ranksrow[$j]['rank_image'] ) ? '<img align="absmiddle" src="' . $ranksrow[$j]['rank_image'] . '" alt="' . $poster_rank . '" title="' . $poster_rank . '" border="0" /><br />' : '';
			}
		}
	}
	else
	...
	if ( $poster_id != ANONYMOUS )
	{
		...
		$hb_pm = '<a href="' . $temp_url . '">' . $lang['Send_private_message'] . '</a>&nbsp;';
	...
	$hb_quote = '<a href="' . $temp_url . '">' . $lang['Reply_with_quote'] . '</a>&nbsp;|&nbsp;';
	...
	if ( ( $userdata['user_id'] == $poster_id && $is_auth['auth_edit'] ) || $is_auth['auth_mod'] )
	{
		...
		$hb_edit = '<a href="' . $temp_url . '">' . $lang['Edit_delete_post'] . '</a>&nbsp;|&nbsp;';
	}
	else
	{
		$edit_img = '';
	...
	if ( $is_auth['auth_mod'] )
	{
		...
		$hb_ip = $ip = '|&nbsp;<a href="' . $temp_url . '">' . $lang['View_IP'] . '</a>';

		$temp_url = "posting.$phpEx?mode=delete&amp;" . POST_POST_URL . "=" . $postrow[$i]['post_id'] . "&amp;sid=" . $userdata['session_id'];
		$delpost_img = '<a href="' . $temp_url . '"><img src="' . $images['icon_delpost'] . '" alt="' . $lang['Delete_post'] . '" title="' . $lang['Delete_post'] . '" border="0" /></a>';
		$delpost = '<a href="' . $temp_url . '">' . $lang['Delete_post'] . '</a>';
		$hb_delpost = '<a href="' . $temp_url . '">' . $lang['Delete_post'] . '</a>&nbsp;|&nbsp;';
	}
	else
	{
		$ip_img = '';
	...
	if ( $user_sig != '' )
	{
		$user_sig = '<br /><span class="SignatureTitle"><b>-----signature-----</b></span><br />' . str_replace("\n", "\n<br />\n", $user_sig);
	}
	...
	if ( $postrow[$i]['post_edit_count'] )
	{
		...
		$hb_edited_by = '<b>Date Edited:</b>&nbsp;' . sprintf(create_date($board_config['default_dateformat'], $postrow[$i]['post_edit_time'], $board_config['board_timezone'])) . '&nbsp;(edits total:&nbsp;' . $postrow[$i]['post_edit_count'] . ')&nbsp;<b>Edited By:</b>&nbsp;' . sprintf($poster);
	}
	else
	{
		$l_edited_by = '';
	...
	$template->assign_block_vars('postrow', array(
		...
		'ROW_CLASS' => $row_class,
		'POSTER_NAME' => $poster,
		'HB_POSTER_NAME' => $hb_poster,
		'POSTER_RANK' => $poster_rank,
		'RANK_IMAGE' => $rank_image,
		'HB_RANK_IMAGE' => $hb_rank_image,
		...
		'HB_EDITED_MESSAGE' => $hb_edited_by,
