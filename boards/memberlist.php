...
$sql = "SELECT username, user_id, user_rank, user_posts, user_viewemail, user_posts, user_regdate, user_from, user_website, user_email, user_icq, user_aim, user_yim, user_msnm, user_avatar, user_avatar_type, user_allowavatar 
	FROM " . USERS_TABLE . "
...
if ( $row = $db->sql_fetchrow($result) )
{
	...
	do
	{
		$username = get_ranked_username($row['user_id'],$row['username'],$row['user_rank'],$row['user_posts']);
		$user_id = $row['user_id'];
		...
		if ( !empty($row['user_viewemail']) || $userdata['user_level'] == ADMIN )
		{
			...
			$hb_email ='<a href="' . $email_uri . '">' . $row['user_email'] . '</a>';
		}
		else
		{
			$email_img = '&nbsp;';
		...
		$hb_pm = '<a href="' . $temp_url . '">' . $lang['Send_PM'] . '</a>';

		$www_img = ( $row['user_website'] ) ? '<a href="' . $row['user_website'] . '" target="_userwww"><img src="' . $images['icon_www'] . '" alt="' . $lang['Visit_website'] . '" title="' . $lang['Visit_website'] . '" border="0" /></a>' : '';
		$www = ( $row['user_website'] ) ? '<a href="' . $row['user_website'] . '" target="_userwww">' . $lang['Visit_website'] . '</a>' : '';
		$hb_www = ( $row['user_website'] ) ? '<a href="' . $row['user_website'] . '" target="_userwww">' . $lang['Link'] . '</a>' : '';
