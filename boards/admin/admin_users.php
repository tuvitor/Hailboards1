...
attachment_quota_settings('user', $HTTP_POST_VARS['submit'], $mode);
	...
	if ( ( $mode == 'save' && isset( $HTTP_POST_VARS['submit'] ) ) || isset( $HTTP_POST_VARS['avatargallery'] ) || isset( $HTTP_POST_VARS['submitavatar'] ) || isset( $HTTP_POST_VARS['cancelavatar'] ) )
	{
		...
		$real_name = ( !empty($HTTP_POST_VARS['real_name']) ) ? trim(strip_tags( $HTTP_POST_VARS['real_name'] ) ) : '';
                ...
                $picture = ( !empty($HTTP_POST_VARS['picture']) ) ? trim(strip_tags( $HTTP_POST_VARS['picture'] ) ) : '';
	...
	else if( !isset( $HTTP_POST_VARS['submit'] ) && $mode != 'save' && !isset( $HTTP_POST_VARS['avatargallery'] ) && !isset( $HTTP_POST_VARS['submitavatar'] ) && !isset( $HTTP_POST_VARS['cancelavatar'] ) )
	{
		...
		$interests = htmlspecialchars($this_userdata['user_interests']);
                ...
                $picture = htmlspecialchars($this_userdata['user_picture']);
	...
	if( isset($HTTP_POST_VARS['avatargallery']) && !$error )
	{
		if( !$error )
		{
			...
			$s_hidden_fields .= '<input type="hidden" name="real_name" value="' . str_replace("\"", "&quot;", $real_name) . '" />';
                        $s_hidden_fields .= '<input type="hidden" name="gender" value="' . str_replace("\"", "&quot;", $gender) . '" />';
                        $s_hidden_fields .= '<input type="hidden" name="company" value="' . str_replace("\"", "&quot;", $company) . '" />';
                        $s_hidden_fields .= '<input type="hidden" name="picture" value="' . str_replace("\"", "&quot;", $picture) . '" />';
