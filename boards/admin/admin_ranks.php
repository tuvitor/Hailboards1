...
if( $mode != "" )
{
	if( $mode == "edit" || $mode == "add" )
	{
		...
		$template->assign_vars(array(
			...
			"TAG_OPEN" => htmlentities($rank_info['tag_open']),
			"TAG_CLOSE" => htmlentities($rank_info['tag_close']),
			...
			"L_RANK_IMAGE_EXPLAIN" => $lang['Rank_image_explain'],
		        "L_TAG_CLOSE" => $lang['Tag_close'],
         		"L_TAG_OPEN" => $lang['Tag_open'],
	...
	else if( $mode == "save" )
	{
		...
		$tag_open = ( (isset($HTTP_POST_VARS['tag_open'])) ) ? trim($HTTP_POST_VARS['tag_open']) : "";
      		$tag_close = ( (isset($HTTP_POST_VARS['tag_close'])) ) ? trim($HTTP_POST_VARS['tag_close']) : ""; 
		...
		if ($rank_id)
		{
			...
			$sql = "UPDATE " . RANKS_TABLE . "
                                SET rank_title = '" . str_replace("\'", "''", $rank_title) . "', rank_special = $special_rank, rank_min = $min_posts, rank_image = '" . str_replace("\'", "''", $rank_image) . "', tag_open = '" . str_replace("\'", "''", $tag_open) . "', tag_close = '" . str_replace("\'", "''" ,$tag_close) . "'
                                WHERE rank_id = $rank_id";
                ...
                {
                        $sql = "INSERT INTO " . RANKS_TABLE . " (rank_title, rank_special, tag_open, tag_close, rank_min, rank_image)
                                VALUES ('" . str_replace("\'", "''", $rank_title) . "', $special_rank, '" . str_replace("\'", "''", $tag_open) . "', '" . str_replace("\'", "''", $tag_close) . "', $min_posts, '" . str_replace("\'", "''", $rank_image) . "')";
