...
elseif( isset($HTTP_GET_VARS['pane']) && $HTTP_GET_VARS['pane'] == 'right' )
{
	...
	$current_version = explode('.', '2' . $board_config['hb_version']);
	$minor_revision = (int) $current_version[2];

	$errno = 0;
	$errstr = $version_info = '';

	if ($fsock = @fsockopen('hailboards.epicfactory.com', 80, $errno, $errstr, 10))
	{
		@fputs($fsock, "GET /updatecheck/12x.txt HTTP/1.1\r\n");
		@fputs($fsock, "HOST: hailboards.epicfactory.com\r\n");
		...
		else
		{
			...
			$version_info .= '<br />' . sprintf($lang['Latest_version_info'], $latest_version) . ' ' . sprintf($lang['Current_version_info'], $board_config['hb_version']) . '</p>';
		}
	}
	else
	{
