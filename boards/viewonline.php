...
// Get user list
//
$sql = "SELECT u.user_id, u.user_rank, u.user_posts, u.username, u.user_allow_viewonline, u.user_level, s.session_logged_in, s.session_time, s.session_page, s.session_ip
...
while ( $row = $db->sql_fetchrow($result) )
{
	...
	if ( $view_online )
	{
		...
		$template->assign_block_vars("$which_row", array(
			'ROW_COLOR' => '#' . $row_color,
			'ROW_CLASS' => $row_class,
			'USERNAME' => get_ranked_username($user_id,$row['username'],$row['user_rank'],$row['user_posts']),
